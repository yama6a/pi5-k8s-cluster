#!/usr/bin/env bash
# Restores an opt-in Longhorn volume from its S3 backups. Use it when the data is genuinely gone.
# The CSI snapshotter sidecar is DISABLED on this cluster, so the VolumeSnapshot restore path is
# unavailable. Instead: a Longhorn Volume CR with spec.fromBackup pulls the backup into a NEW volume, then a
# static PV and PVC bind to it in the target namespace. Point your workload at the restored PVC.
...
#
# Usage (flags optional, prompts or lists for anything missing):
...
# volume size (bytes) for the Volume/PV/PVC capacity, taken from the BackupVolume and falling back to the Backup
